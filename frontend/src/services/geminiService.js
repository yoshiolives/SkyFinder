...
const handleAddActivity = (userMessage, itinerary) => {
  ...
  if (message.includes('central park')) {
    ...
    newActivity.coordinates = [40.7480, -74.0048];
    ...
    newActivity.coordinates = [40.7580, -73.9855];
    ...
    // Handle ice cream shop requests
    if (message.includes('new york') || message.includes('nyc') || message.includes('manhattan')) {
    ...
    // Handle Vancouver museum requests - actually add the museum
    if (message.includes('museum of anthropology') || message.includes('anthropology')) {
      ...
      // Default to top Vancouver museum (Museum of Anthropology)
      newActivity.location = 'Museum of Anthropology';
      newActivity.address = '6393 NW Marine Dr, Vancouver, BC V6T 1Z2, Canada';
      newActivity.activity = 'First Nations Art & Culture Tour';
      newActivity.coordinates = [49.2697, -123.2604];
...
const handleRemoveActivity = (userMessage, itinerary) => {
  // Find activity to remove based on user message
  let activityToRemove = null;
  
  if (userMessage.toLowerCase().includes('central park')) {
    activityToRemove = itinerary.find(item => item.location.toLowerCase().includes('central park'));
  } else if (userMessage.toLowerCase().includes('metropolitan')) {
    activityToRemove = itinerary.find(item => item.location.toLowerCase().includes('metropolitan'));
  } else if (userMessage.toLowerCase().includes('times square')) {
    activityToRemove = itinerary.find(item => item.location.toLowerCase().includes('times square'));
...
const handleModifyActivity = (userMessage, itinerary) => {
  return {
...
const handleReorderActivities = (userMessage, itinerary) => {
  return {
...
const getGeminiResponse = async (userMessage, itinerary = []) => {
  try {
    ...
    // Itinerary modification patterns - improved logic
    if (message.includes('add') || message.includes('can you add') || message.includes('add it to')) {
    ...
    if (message.includes('suggest') || message.includes('recommend') || message.includes('advice')) {
      return {
...
const getRealGeminiResponse = async (userMessage, itinerary = []) => {
  ...
  const prompt = `You are a helpful travel assistant. The user is planning a trip to New York City with the following itinerary: ${JSON.stringify(itinerary)}. 
  User message: ${userMessage}
  
  Please provide a helpful response about their travel plans.`;
  ...
  */
  ...
  return getGeminiResponse(userMessage, itinerary);
};

export { getGeminiResponse, getRealGeminiResponse };
